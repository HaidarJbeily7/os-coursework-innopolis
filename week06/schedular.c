...
void read_file(FILE *file)
{
    ...
    {
        sscanf(str, "%d %d %d", &data[data_size].idx, &data[data_size].at, &data[data_size].bt);
...
void schedule_handler(int signum)
{
    ...
    if (running_process >= 0 && ps[running_process] > 0)
    {
        ...
        if (data[running_process].burst == 0)
        {
            ...
            waitpid(ps[running_process], NULL, 0); // Wait for process termination
                                                   // Calculate metrics like ct, tat, wt
